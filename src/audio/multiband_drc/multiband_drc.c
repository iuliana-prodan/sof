...
static inline void multiband_drc_crossover_reset_state(struct multiband_drc_state *state)
{
	int i;
...
static inline void multiband_drc_emphasis_reset_state(struct multiband_drc_state *state)
{
	int i;

	for (i = 0; i < PLATFORM_MAX_CHANNELS; i++)
		crossover_reset_state_lr4(&state->emphasis[i]);
}

static inline void multiband_drc_deemphasis_reset_state(struct multiband_drc_state *state)
{
	int i;

	for (i = 0; i < PLATFORM_MAX_CHANNELS; i++)
		crossover_reset_state_lr4(&state->deemphasis[i]);
}

static inline void multiband_drc_drc_reset_state(struct multiband_drc_state *state)
{
	int i;
...
static inline void multiband_drc_reset_state(struct multiband_drc_state *state)
{
	...
	multiband_drc_emphasis_reset_state(state);

	/* Reset crossover state */
	multiband_drc_crossover_reset_state(state);

	/* Reset drc kernel state */
	multiband_drc_drc_reset_state(state);

	/* Reset deemphasis eq-iir state */
	multiband_drc_deemphasis_reset_state(state);
}
					 ...
					 struct iir_state_df2t *eq)
{
	...
	ret = memcpy_s(eq->coef, sizeof(struct sof_eq_iir_biquad_df2t) * 2,
		       coef, sizeof(struct sof_eq_iir_biquad_df2t) * 2);
	assert(!ret);
	...
	eq->biquads = 2;
	eq->biquads_in_series = 2;

	return 0;
}

static int multiband_drc_crossover_eq_init_coef(struct multiband_drc_comp_data *cd, int16_t nch)
{
	...
	if (!config) {
		comp_cl_err(&comp_multiband_drc, "crossover_eq_init_coef(), no config is set");
		return -EINVAL;
	}
	...
	if (nch > PLATFORM_MAX_CHANNELS) {
		comp_cl_err(&comp_multiband_drc,
			    "crossover_eq_init_coef(), invalid channels count(%i)",
			    nch);
		return -EINVAL;
	}

	comp_cl_info(&comp_multiband_drc, "crossover_eq_init_coef(), initiliazing %i-way crossover",
		     config->num_bands);

	/* Crossover: collect the coef array and assign it to every channel */
	crossover = config->crossover_coef;
	for (ch = 0; ch < nch; ch++) {
		err = crossover_init_coef_ch(crossover, &state->crossover[ch],
					     config->num_bands);
		/* Free all previously allocated blocks in case of an error */
		if (err < 0) {
			comp_cl_err(&comp_multiband_drc,
				    "crossover_eq_init_coef(), could not assign coeffs to ch %d",
				    ch);
			multiband_drc_crossover_reset_state(state);
			return err;
		}
	...
	comp_cl_info(&comp_multiband_drc, "crossover_eq_init_coef(), initiliazing emphasis_eq");

	/* Emphasis: collect the coef array and assign it to every channel */
	emphasis = config->emp_coef;
	for (ch = 0; ch < nch; ch++) {
		err = multiband_drc_eq_init_coef_ch(emphasis, &state->emphasis[ch]);
		/* Free all previously allocated blocks in case of an error */
		if (err < 0) {
			comp_cl_err(&comp_multiband_drc,
				    "crossover_eq_init_coef(), could not assign coeffs to ch %d",
	...
	comp_cl_info(&comp_multiband_drc, "crossover_eq_init_coef(), initiliazing deemphasis_eq");

	/* Deemphasis: collect the coef array and assign it to every channel */
	deemphasis = config->deemp_coef;
	for (ch = 0; ch < nch; ch++) {
		err = multiband_drc_eq_init_coef_ch(deemphasis, &state->deemphasis[ch]);
		/* Free all previously allocated blocks in case of an error */
		if (err < 0) {
			comp_cl_err(&comp_multiband_drc,
				    "crossover_eq_init_coef(), could not assign coeffs to ch %d",
...
static int multiband_drc_drc_init(struct multiband_drc_comp_data *cd, int16_t nch, uint32_t rate)
{
	...
	for (i = 0; i < SOF_MULTIBAND_DRC_MAX_BANDS; i++) {
		...
		comp_cl_info(&comp_multiband_drc, "drc_init(), initiliazing drc band %d", i);

		ret = drc_init_pre_delay_buffers(&state->drc[i], (size_t)sample_bytes, (int)nch);
		if (ret < 0) {
			comp_cl_err(&comp_multiband_drc,
				    "drc_init(), could not init pre delay buffers");
			return ret;
		}
		...
		if (ret < 0) {
			comp_cl_err(&comp_multiband_drc,
				    "drc_init(), could not set pre delay time");
			return ret;
		}
	}
...
static int multiband_drc_setup(struct multiband_drc_comp_data *cd, int16_t channels, uint32_t rate)
{
	...
	/* Setup Crossover, Emphasis EQ, and Deemphasis EQ */
	ret = multiband_drc_crossover_eq_init_coef(cd, channels);
	if (ret < 0)
		return ret;

	/* Setup Multiband DRC */
	ret = multiband_drc_drc_init(cd, channels, rate);
	if (ret < 0)
		return ret;
				      ...
				      int max_size)
{
	...
	case SOF_CTRL_CMD_BINARY:
		comp_info(dev, "multiband_drc_cmd_get_data(), SOF_CTRL_CMD_BINARY");
				      ...
				      struct sof_ipc_ctrl_data *cdata)
{
	...
	switch (cdata->cmd) {
	case SOF_CTRL_CMD_BINARY:
		comp_info(dev, "multiband_drc_cmd_set_data(), SOF_CTRL_CMD_BINARY");
...
static int multiband_drc_cmd(struct comp_dev *dev, int cmd, void *data, int max_data_size)
{
	...
	comp_info(dev, "multiband_drc_cmd()");
...
static int multiband_drc_trigger(struct comp_dev *dev, int cmd)
{
	...
	comp_info(dev, "multiband_drc_trigger()");
